...
func main() {
	...
	wg.Add(1)
	go core.Transmit4into1chan(&mu, &wg, chanResultsHttp, chanResults, &countToCloseChan, maxGoroutines)
	wg.Add(1)
	go core.Transmit4into1chan(&mu, &wg, chanResultsDNS, chanResults, &countToCloseChan, maxGoroutines)
	wg.Add(1)
	go core.Transmit4into1chan(&mu, &wg, chanResultsAmass, chanResults, &countToCloseChan, maxGoroutines)
	wg.Add(1)
	go core.Transmit4into1chan(&mu, &wg, chanResultsSubfinder, chanResults, &countToCloseChan, maxGoroutines)

	wg.Add(1)
	go core.ScanDomain(ctx, &wg, workDirectory, domainName, chanResults)
